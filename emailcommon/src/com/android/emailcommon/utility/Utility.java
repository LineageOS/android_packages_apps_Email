        ...
        return EmailAsyncTask.runAsyncParallel(new Runnable() {
            private void updateLastSeenMessageKeyForAccount(long accountId, long mailboxId) {
                ...
                if (accountId == Account.ACCOUNT_ID_COMBINED_VIEW) {
                    ...
                    //		Mailbox.ID_PROJECTION, null, null, null);
                	Mailbox mailbox =
                        Mailbox.restoreMailboxWithId(context, mailboxId); // c.getLong(Mailbox.ID_PROJECTION_COLUMN));
                	// mailbox has been removed
        			if (mailbox == null) {
        				return;
        			}
        			...
        			// value, we are claiming that every message before this has potentially been
        			// seen by the user.
        			long messageId = Utility.getFirstRowLong(context,
        					Message.CONTENT_URI,
        			...
        			long oldLastSeenMessageId = Utility
        					.getFirstRowLong(
        							context,
        							ContentUris.withAppendedId(
        									Mailbox.CONTENT_URI,
        			...
        			if (messageId != oldLastSeenMessageId) {
        				ContentValues values = mailbox.toContentValues();
        				values.put(MailboxColumns.LAST_SEEN_MESSAGE_KEY,
        						messageId);
        				resolver.update(Mailbox.CONTENT_URI, values,
        						EmailContent.ID_SELECTION,
        						new String[] { Long.toString(mailbox.mId) });
        			}
                }
