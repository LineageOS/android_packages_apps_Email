...
public class ImapService extends Service {
    ...
    private static class ImapIdleListener implements ImapFolder.IdleCallback {
        ...
        @Override
        public void onNewServerChange(final boolean needSync, final boolean needFullSync,
                final List<String> fetchMessages) {
            ...
            sExecutor.execute(new Runnable() {
                ...
                public void run() {
                    ...
                    processImapIdleChangesLocked(mContext, mStore.getAccount(), mMailbox,
                            needSync, needFullSync, fetchMessages);
                }
            });
        }
    ...
    private static class ImapIdleFolderHolder {
        ...
        private boolean registerMailboxForIdle(Context context, Account account, Mailbox mailbox)
            ...
            synchronized (mIdledFolders) {
                ...
                try {
                    ...
                    final ImapFolder idledFolder =
                            (ImapFolder) remoteStore.getFolder(mailbox.mServerId);
                    idledFolder.open(OpenMode.READ_WRITE);
                    idledFolder.startIdling(new ImapIdleListener(context, remoteStore, mailbox));
        ...
        private void unregisterIdledMailbox(long mailboxId) throws MessagingException {
            synchronized (mIdledFolders) {
                ...
                // Stop idling
                final ImapFolder idleFolder = mIdledFolders.valueAt(index);
                mIdledFolders.removeAt(index);
                idleFolder.stopIdling();
...
//            final long queryEndDate = endDate - DateUtils.DAY_IN_MILLIS - DateUtils.HOUR_IN_MILLIS;
    ...
    private static synchronized void processImapIdleChangesLocked(Context context, Account account,
            Mailbox mailbox, boolean needSync, boolean needFullSync, List<String> fetchMessages) {
        ...
        if (Logging.LOGD) {
            LogUtils.d(LOG_TAG, "Processing IDLE changes for mailbox " + mailbox.mId
                    + ": need sync " + needSync + "(full " + needFullSync + "), "
        ...
        try {
            ...
            if (msgToFetchSize > 0 && msgToFetchSize < MAX_MESSAGES_TO_FETCH
                    && !(needSync && needFullSync)) {
                processImapFetchChanges(context, account, mailbox, fetchMessages);
                syncRequested = true;
            }
            if (needSync || msgToFetchSize > MAX_MESSAGES_TO_FETCH) {
                requestSync(context, account, mailbox.mId, needFullSync);
                syncRequested = true;
            }
        } catch (MessagingException ex) {
