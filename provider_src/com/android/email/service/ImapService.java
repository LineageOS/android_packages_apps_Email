...
import com.android.email.mail.store.ImapFolder.ImapIdleChanges;
...
public class ImapService extends Service {
    ...
    private static class ImapIdleListener implements PushCallback {
        ...
        @Override
        public void onNewServerChange(Object changes) {
            ...
            final ImapIdleChanges pendingChanges = (ImapIdleChanges) changes;
            sExecutor.execute(new Runnable() {
                @Override
                public void run() {
                    // Selectively process all the retrieved changes
                    processImapIdleChangesLocked(mContext,
                            mStore.getAccount(), mMailbox, pendingChanges);
                }
            });
        }
    ...
    private static class ImapIdleFolderHolder {
        ...
        private synchronized Store getStore(Context context, Account account)
                throws MessagingException {
            Store store = mStores.get((int) account.mId);
            if (store == null) {
                store = Store.getInstance(account, context);
                mStores.put((int) account.mId, store);
            }
            return store;
        }
        ...
        private boolean registerMailboxForIdle(Context context, Account account, Mailbox mailbox)
            ...
            synchronized (mIdledFolders) {
                ...
                try {
                    Store remoteStore = getStore(context, account);
        ...
        private void unregisterAllIdledMailboxes(boolean disconnect) {
            synchronized (mIdledFolders) {
                ...
                if (disconnect) {
                    count = mStores.size() - 1;
                    for (int index = count; index >= 0; index--) {
                        final Store store = mStores.valueAt(index);
    ...
    private static class LocalChangesContentObserver extends ContentObserver {
        ...
        private void processMessageChanged(String op, long id) {
            ...
            try {
                final ImapIdleFolderHolder holder = ImapIdleFolderHolder.getInstance();
                Store remoteStore = holder.getStore(mContext, account);
...
//            final long queryEndDate = endDate - DateUtils.DAY_IN_MILLIS - DateUtils.HOUR_IN_MILLIS;
    ...
    private static synchronized void processImapIdleChangesLocked(
            Context ctx, Account acct, Mailbox mailbox, ImapIdleChanges changes) {
        ...
        final boolean sync = changes.mRequiredSync;
        final boolean full = changes.mFullSync;
        final int msgToFetchSize = changes.mMessageToFetch.size();
        ...
        try {
            ...
            if (msgToFetchSize > 0 && msgToFetchSize < MAX_MESSAGES_TO_FETCH && !(sync && full)) {
                processImapFetchChanges(ctx, acct, mailbox, changes.mMessageToFetch);
                syncRequested = true;
            }
            if (sync || msgToFetchSize > MAX_MESSAGES_TO_FETCH) {
                requestSync(ctx, acct, mailbox.mId, full);
                syncRequested = true;
            }
        } catch (MessagingException ex) {
        ...
        try {
            if (!syncRequested && acct.getSyncInterval() == Account.CHECK_INTERVAL_PUSH) {
                final ImapIdleFolderHolder holder = ImapIdleFolderHolder.getInstance();
                holder.registerMailboxForIdle(ctx, acct, mailbox);
            }
        } catch (MessagingException ex) {
